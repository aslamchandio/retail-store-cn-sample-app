...
resource "google_container_cluster" "gke_cluster" {
  ...
  subnetwork         = data.terraform_remote_state.vpc.outputs.subnet1_id
  ...
  ip_allocation_policy {
    stack_type                    = "IPV4"
    cluster_secondary_range_name  = data.terraform_remote_state.vpc.outputs.pod_cidr1_name
    services_secondary_range_name = data.terraform_remote_state.vpc.outputs.service_cidr_name
  }
  ...
  master_authorized_networks_config {
    ...
    cidr_blocks {
      ...
      cidr_block   = data.terraform_remote_state.vpc.outputs.subnet2_cidr
    }
    cidr_blocks {
      display_name = "ThirdSubnet-CIDR"
      cidr_block   = data.terraform_remote_state.vpc.outputs.subnet3_cidr
    }
      cidr_blocks {
      display_name = "FourthSubnet-CIDR"
      cidr_block   = var.google_vm_ip
    }

    gcp_public_cidrs_access_enabled = true # Enable access to GCP public IP ranges (e.g. for Cloud Logging, Cloud Monitoring, etc.)
